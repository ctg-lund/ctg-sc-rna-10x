...
usage() {
    ...
    echo "projid,<PROJID>"
    echo "email,<email recipient adress>"
...
if [ -f $delinfo ]; then
    email_cust=$(grep "email," $delinfo | cut -f2 -d"," | tr -d '\n\r' | sed "s/;/ /g")
    pid=$(grep "projid," $delinfo | cut -f2 -d"," | tr -d '\n\r')
    pipe=$(grep "pipeline," $delinfo | cut -f2 -d"," | tr -d '\n\r')

    if [ -z "$email_cust" ]; then
	echo "> Error: no customer/recipient email found in $delinfo.. this file needs a row with 'email,<recipient mail adress>'"
	exit_abnormal
    fi 
    if [ -z $pid ]; then
...
emaitemplate="/projects/fs1/shared/ctg-pipelines/ctg-${pipe}/ctg-delivery-mail/ctg-delivery-mail_${pipe}.csv"
...
# Define CC
# Add Julia and Liesl and CTGBNFmail (defined above) as CC, so they also are on the delivery-email list.
cc="-c ulrich.pfisterer@med.lu.se julia.braunig@med.lu.se liesl.joubert@med.lu.se $ctgmail"
# check if there are other CTG members that should be cc'ed
echo "> CTG CC: $cc"

# Check if no customer specified- then add cc as email_cust
if [ -z "$email_cust" ]; then
    echo "> No customer email specified. Then send only to ctg : $cc"
    email_cust="$cc"
    cc=""
else
...
mailcmd="echo '' | mutt -s 'CTG $pipe delivery of $pid' $email_cust -i /srv/data/$customer/$newmail -e 'unmy_hdr from; my_hdr From: ${ctgName} <${ctgmail}>' -e 'set content_type=text/html' ${cc} $att"
